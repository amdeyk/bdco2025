...
def create_simple_badge_common(guest: dict) -> Image.Image:
    ...
    try:
        ...
        try:
            ...
            draw.text((width//2, title_y), config.get('CONFERENCE', 'Name'), fill='white', anchor="mm")
            draw.text((width//2, title_y + 30), config.get('CONFERENCE', 'Tagline'), fill='white', anchor="mm")
            draw.text((width//2, title_y + 55), config.get('CONFERENCE', 'Dates'), fill='white', anchor="mm")
        except:
            # Fallback if font issues
            draw.text((50, content_margin + 40), f"{config.get('CONFERENCE','Name')} - {config.get('CONFERENCE','Tagline')}", fill='white')
...
def create_magnacode_badge_working(guest: dict) -> Image.Image:
    ...
    qr.add_data(f"{config.get('CONFERENCE', 'QRPrefix')}:{guest.get('ID', 'UNKNOWN')}")
    qr.make(fit=True)
...
async def submit_abstract(
    ...
    # Email notifications are disabled in this project

    return templates.TemplateResponse(
        "abstract_submission.html",
        {
            "request": request,
            "success": "Abstract submitted successfully.",
