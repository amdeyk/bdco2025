...
# Initialize services
config = Config()
templates.env.globals["conference_name"] = config.get('CONFERENCE', 'Name')
templates.env.globals["conference_tagline"] = config.get('CONFERENCE', 'Tagline')
templates.env.globals["conference_dates"] = config.get('CONFERENCE', 'Dates')
templates.env.globals["conference_venue"] = config.get('CONFERENCE', 'Venue')
templates.env.globals["conference_city"] = config.get('CONFERENCE', 'City')
templates.env.globals["conference_website"] = config.get('CONFERENCE', 'Website')
templates.env.globals["conference_contact_email"] = config.get('CONFERENCE', 'ContactEmail')
templates.env.globals["support_phone1"] = config.get('CONFERENCE', 'SupportPhone1')
templates.env.globals["support_phone2"] = config.get('CONFERENCE', 'SupportPhone2')
templates.env.globals["schedule_file"] = config.get('CONFERENCE', 'ScheduleFile')
...
async def download_badge(admin: Dict = Depends(get_current_admin), guest_id: str = FastAPIPath(...)):
    ...
    try:
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f'attachment; filename="{config.get("CONFERENCE","QRPrefix")}_Badge_{guest_id}.png"'
            }
        )
...
async def generate_badge(admin: Dict = Depends(get_current_admin), guest_id: str = FastAPIPath(...)):
    ...
    try:
        ...
        return StreamingResponse(
            ...
            media_type="image/png",
            headers={
                "Content-Disposition": f'attachment; filename="{config.get("CONFERENCE","QRPrefix")}_{guest_id}_badge.png"'
            }
        )
...
async def download_all_badges(admin: Dict = Depends(get_current_admin)):
    ...
    try:
        ...
        archive_name = f"{config.get('CONFERENCE','QRPrefix')}_badges_{download_started.strftime('%Y%m%d%H%M%S')}.zip"
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as archive:
            for guest in guests:
                ...
                archive.writestr(
                    f"{config.get('CONFERENCE','QRPrefix')}_{guest_id}_badge.png",
...
def create_magnacode_badge(guest: dict) -> Image.Image:
    """Create corporate badge using conference config - NO FALLBACKS"""
    dpi = 300
    ...
    draw.text((width_px//2, 60), config.get('CONFERENCE','Name'), fill='white', anchor="mm", font_size=48)
    draw.text((width_px//2, 120), config.get('CONFERENCE','Tagline'), fill='white', anchor="mm", font_size=28)
    draw.text((width_px//2, 170), config.get('CONFERENCE','Dates'), fill=gold, anchor="mm", font_size=24)
    draw.text((width_px//2, 210), config.get('CONFERENCE','City'), fill='white', anchor="mm", font_size=22)
    ...
    qr.add_data(f"{config.get('CONFERENCE','QRPrefix')}:{guest['ID']}")
    qr.make(fit=True)
...
def create_magnacode_badge_working(guest: dict) -> Image.Image:
    ...
    qr = qrcode.QRCode(version=1, error_correction=qrcode.constants.ERROR_CORRECT_M, box_size=12, border=1)
    qr.add_data(f"{config.get('CONFERENCE','QRPrefix')}:{guest_id}")
    qr.make(fit=True)
    qr_img = qr.make_image(fill_color=navy_blue, back_color="white")
    try:
        qr_resized = qr_img.resize((qr_size, qr_size), Image.Resampling.LANCZOS)
...
def create_simple_badge(guest: Dict) -> Image.Image:
    ...
    try:
        ...
        try:
            # Main title
            draw.text((width//2, 30), config.get('CONFERENCE','Name'), fill='white', anchor="mm", font_size=36)
            ...
            draw.text((50, 30), f"{config.get('CONFERENCE','Name')} - {config.get('CONFERENCE','Dates')}", fill='white')
        ...
        footer_items = [
            f"Venue: {config.get('CONFERENCE','Venue')}, {config.get('CONFERENCE','City')}",
            config.get('CONFERENCE','Tagline'),
            config.get('CONFERENCE','Name')
        ]
...
def create_itinerary_document(guest: Dict) -> str:
    ...
    try:
        content = f"""
===========================================
{config.get('CONFERENCE','QRPrefix')} - JOURNEY ITINERARY
===========================================
...
Conference Details:
- Event: {config.get('CONFERENCE','Name')}
- Dates: {config.get('CONFERENCE','Dates')}
- Venue: {config.get('CONFERENCE','Venue')}, {config.get('CONFERENCE','City')}
- Organizer: {config.get('CONFERENCE','Tagline')}
...
Contact Information:
- Conference Helpline: {config.get('CONFERENCE','SupportPhone1')}
- Email: {config.get('CONFERENCE','ContactEmail')}
